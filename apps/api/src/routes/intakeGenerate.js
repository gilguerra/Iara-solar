import { createIntakeAndGenerate } from '../repositories/intakeGenerateRepository.js';

export async function intakeGenerateRoutes(app) {
  app.post('/intake/generate', async (request, reply) => {
    ...
    try {
      const result = await createIntakeAndGenerate({
      ...
      return reply.code(400).send({ error: 'intake_generate_error', message: error.message });
    }
  });
}
